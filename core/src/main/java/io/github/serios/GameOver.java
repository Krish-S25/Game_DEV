...
public class GameOver implements Screen {
    ...
    public GameOver(final Main game, int levelId, boolean won) {
        this.game = game;
        ...
        if (won) {
            actionButton = new TextButton("Next Level", skin);
            actionButton.addListener(
                new ClickListener() {
                    @Override
                    public void clicked(InputEvent event, float x, float y) {
                        if (levelId == 1) game.setScreen(new Level2Screen(game));
                        if (levelId == 2) game.setScreen(new Level3Screen(game));
                        if (levelId == 3) game.setScreen(new MenuScreen(game));
                    }
                });
        } else {
            actionButton = new TextButton("Retry Level", skin);
            actionButton.addListener(
                new ClickListener() {
                    @Override
                    public void clicked(InputEvent event, float x, float y) {
